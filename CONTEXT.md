...
- **Current task pointer / 当前任务指针**：侧边栏固定区常驻的一行，恒定回答「这本小说现在该做什么」，由项目状态与创作链缺口推导，点击只做导航、不发起 Agent run（到达目的地后由该处既有的主操作二次确认）。它对新手与老作者一样长期有效，**不是新手引导**（不随「出师」消失、不可关闭）；它也不是 Empty guide（后者只在单个文档缺失时出现在中间内容区），不是首启介绍。全书完结或项目无效时不显示。
- **创作链 / Authoring chain**：一本小说从零到完结必经的顺序步骤——立项卡 → 世界观与角色 → 大纲 → 章节正文 → 完结；参考作品是可选增强，越早加越影响后续每一步。它是引擎各命令前置检查所定义的真实依赖顺序，不等同于项目状态的四阶段划分（后者不区分世界观与角色这一步）。
