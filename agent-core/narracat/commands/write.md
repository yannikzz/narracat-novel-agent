...
**一律走这道门，不受 automation_level 控制**：automation_level 管的是大纲那类结构决策要不要逐层过人，管不到「这一章我想怎么写」——而作者对本章的要求只有这一个入口，全自动档跳过它，等于把人从自己的书里请出去。

0. **先看作者这次有没有已经说了话**：prompt 里的「桌面侧用户补充意图」是作者本人对这一章说的话（$ARGUMENTS 只有章号）。App 已经保证**作者没打字时这一句根本不会出现**，所以它一旦出现就是真话，不必猜是不是按钮文案。
   - 这句话里如果是要求恢复开写前确认（「写之前先问我」「恢复开写确认」这类）→ 把 `chapter_gate` 写回 `"ask"`、`chapter_gate_skips` 归 0，然后照常往下走这道门。**这一条先判，早于下面的关门短路**。
   - 否则它就是一条本章要求：摘要里回一句「你说的 {要求} 我记下了」，别装作没看见再问一遍；作者在门里补充的内容与它合并，一并按第 4 点判冲突、按 3a 带进任务书。
1. **门被作者关过时**（`chapter_gate == "off"`，见第 6 点）：跳过第 2-3 点不问作者；若第 0 点收到了本章要求，**仍要执行第 4 点的冲突判断与第 5 点的落盘**，然后进步骤 3。（关掉的是「主动问你」，不是「听你说话」。）
2. 向作者摘要**这一章要写什么**：细纲要点（发生什么、要打出什么）、上一章欠着的钩子、步骤 1 上下文准备时的提醒。三五行，够作者判断即可，不复述整份细纲。
3. AskUserQuestion（header 用「第 {chapter_num} 章」）。**选项按 `chapter_gate_skips` 决定给三条还是四条**——`>= 3` 时才出现第四条：
   - **开始写** —— 按细纲写这一章。
   - **我有要求** —— 这一章我有想法要交代（也可以直接写进下面的自定义回答框，一次说完，不必先点这条）。
   - **先不写** —— 这次先不写。
   - **以后别问了，直接开写**（仅 `chapter_gate_skips >= 3` 时给）—— 见第 6 点。
   按回答分路：
   - 「先不写」→ 终止。**如实告诉作者**：本章的上下文已经准备好留在原地了，下次再写这一章时会先问你「继续还是重来」。（步骤 1 已经落了上下文包与断点，别承诺「什么都没动」。）
   - 「开始写」→ 进第 6 点计数，然后 novel_checkpoint(step=2)，进步骤 3。
   - 「以后别问了，直接开写」→ 按第 6 点关门，然后 novel_checkpoint(step=2)，进步骤 3。
   - 写了自定义回答，或选「我有要求」→ 收下要求，进第 4 点。**选了「我有要求」却没写具体内容时，再发一次 AskUserQuestion 收要求**（给 2-4 个本章可选的方向作选项，作者可用自定义回答框自由写）——**不要用普通对话文本提问**：那样 App 不会把它渲染成问题卡、也不会等作者回答，模型会自问自答直接往下写。
4. 收到要求后判**冲突**：
   - **判据**：只有当要求会推翻细纲里承载后续的东西——伏笔的埋或收、跨章的人物承诺、章末去向、已计划的状态变更——才算冲突。写法、节奏、详略、情绪浓度、对白多少、某段展开还是收着，**都不算冲突**，照办即可。
   - **不冲突** → 一句废话都不多说，进第 5 点。
   - **真冲突** → 先说清代价：这一拍原本在为哪一章的什么服务，改了之后什么会落空。再 AskUserQuestion 两条路：**按细纲写**（放弃这条要求，其余要求照办）/ **按我的要求写**（本章偏离细纲，大纲与后续伏笔可能要跟着调，完成输出里会再提醒一次）。作者选定后进第 5 点。
5. **把要求落盘，再打断点**：Write `.narracat/staging/ch-{三位章号}.request.md`，逐条写下作者这次提的要求（原话意思照录），冲突后作者选了「按细纲写」的那条也写进去并标明「本章不执行」。**顺序不能反**——先 novel_checkpoint(step=2) 再落盘，中间断掉的话恢复会直接跳去步骤 3，而要求只存在于已经消失的会话里，正文和落实回报都会静默丢失。落盘完成后 novel_checkpoint(step=2)，进步骤 3。
6. **计数与关门**（控制状态，不说给作者听）：
   - 作者选「开始写」且一条要求没提 → `.narracat/config.yaml` 的 `chapter_gate_skips` 加 1（字段不存在按 0 起算），用 Edit 写回。
   - 作者提了要求（无论最后按谁的写）→ `chapter_gate_skips` 归 0。
   - 本次给过第四条选项的（`>= 3`），无论作者选哪条都归 0：已经问过一遍，别每章都来。
   - 作者选「以后别问了，直接开写」→ `chapter_gate` 写成 `"off"`、计数归 0，并告诉作者：以后写章节我直接开写；想让我恢复开写前先问你，写章节时在输入框里说一句「写之前先问我」就行。
...
3. 按下面的五段结构，把包与素材翻译成一份自然语言任务书，Write 到 `.narracat/staging/ch-{三位章号}.brief.md`（三位补零，完整文件名形如 `ch-004.brief.md`——章号段与正文一致，后缀不同，别写成正文的 `ch-004.md`）。铁律：
   ...
   - **作者对本章的要求，按内容归进对应段落如实承载**：先看 `.narracat/staging/ch-{三位章号}.request.md` 存不存在（步骤 2 第 5 点落的；不存在说明作者这次没提要求，跳过本条）。**以这个文件为准、不靠会话记忆**——断点恢复回到这里时，原来的会话早没了。剧情与事件类进第二段、人物类进第三段、写法与节奏与对白类进第四段、章末落点类进第五段。写手只读这份任务书，要求必须长在它该在的地方——单独另起一段会变成一句谁都不认领的口号。原话的意思不打折、不概括成「作者希望写得更精彩」。文件里标了「本章不执行」的那条（冲突后作者选了按细纲写）不写进任务书。逐条记下每条要求落进了哪一段，完成输出要回报。
...
**若 `.narracat/staging/ch-{三位章号}.request.md` 存在**（作者本章提过要求），另起一段逐条回报它们落到哪儿了：Read 该文件与 {manuscript_path}（没有这个文件的章不读正文、不多花这一趟），逐条说这条要求写在正文的哪一段、怎么体现的。做到就说做到，**没做到就说没做到**——写手没接住、或标了「本章不执行」的，都如实说，不用「已充分考虑」这类话糊过去。作者据此才知道下次还值不值得提要求。冲突后按作者要求写的，这里再提醒一句：本章已偏离细纲的哪一点，后续大纲或伏笔可能要跟着调。
