...
1. 确认 `.narracat/config.yaml` 存在；不存在 → 告知作者桌面版新建作品时会自动备好项目文件，缺失说明目录不完整（可回作品列表重建或检查目录是否被移动），命令行用户运行 `/narracat:init`，终止（工具报错不算不存在，如实报错即可）
2. 确认 `bible/references/` 目录存在；不存在 → 提示作者先在桌面版添加参考作品（命令行用户运行 `/narracat:init`），终止
