...
import { STRUCTURE_STAGES } from "../packs/pack-manifest.js";
...
import { loadAliasMap, resolveCharacterUid, normalizeName, } from "./alias-map.js";
...
import { FACT_VALID_AT_SQL, FACT_LATEST_ORDER_SQL, isFactValidAt, SECRET_FILTER_SQL, isSecretHiddenForChat, } from "./fact-temporal.js";
import { foldCharacterCard, renderCardHumanMap, isEmptyFoldedCard, readPredicateFromCard, normalizeExtraValue, } from "./character-card-fold.js";
import { loadStateVocabulary } from "./state-dimensions.js";
import { computeDerivedRelationships, loadFactCountByUid, loadValidRelationshipEdges, } from "./derived-relationships.js";
 ...
 */
export function getChapterWordCountRange(wordsPerChapter) {
    if (wordsPerChapter) {
 ...
 */
function secretFilterClause(ctx) {
    return ctx.secretFilter ? ` AND ${SECRET_FILTER_SQL}` : "";
}
/** 已入库的最新章号（无章时 null） */
function latestChapter(ctx) {
    const row = ctx.db
...
/** 确定性章节距离衰减 */
function decayScore(score, sourceChapter, referenceChapter) {
    if (sourceChapter === null || referenceChapter === null)
        return score;
    return score * (1 / (1 + 0.05 * Math.abs(referenceChapter - sourceChapter)));
}
/** 检索命中回表：取内容与来源章号（失效事实直接丢弃） */
function getSourceDetail(ctx, sourceTable, sourceId) {
    switch (sourceTable) {
        ...
        case "facts": {
            const row = ctx.db
                ...
                .get(sourceId);
            if (!row || row.invalidated_at_chapter !== null)
                return null;
            ...
            if (isSecretHiddenForChat(row, ctx.secretFilter ?? false))
                return null;
            return {
        ...
        case "arc_summaries": {
            ...
            if (sep === -1)
                return null;
            const row = ctx.db
                .prepare("SELECT * FROM arc_summaries WHERE novel_id = ? AND scope = ? AND scope_id = ?")
                .get(ctx.novelId, sourceId.slice(0, sep), sourceId.slice(sep + 1));
            if (!row)
                return null;
...
function getArcForChapter(ctx, chapter) {
    return ctx.db
        .prepare(`SELECT * FROM arc_meta
       WHERE novel_id = ? AND chapter_start <= ? AND chapter_end >= ?
       ORDER BY chapter_start LIMIT 1`)
        .get(ctx.novelId, chapter, chapter);
}
function getVolumeRange(ctx, volumeNo) {
    const row = ctx.db
 ...
 */
async function arcRecall(ctx, query, limit) {
...
function multiHopFallback(ctx, query, seedEntities, limit) {
 ...
 */
async function multiHopRecall(ctx, query, seedEntities, aliasMap, limit) {
    ...
    const facts = ctx.db
        .prepare(`SELECT id, subject_character_uid, subject_character_b_uid, predicate, object
       FROM facts
...
export async function novelQuery(args, ctx) {
    const query = args["query"];
    if (typeof query !== "string" || !query.trim()) {
        return singleError("query", "non-empty string", describe(query), "传入查询短语，宜短宜具体（人名 / 物件名 / 事件短语），或一句话问题");
    }
...
export async function novelChapterSummary(args, ctx) {
    ...
    if (chapter !== null) {
        rows = ctx.db
            .prepare("SELECT * FROM chapter_summaries WHERE novel_id = ? AND chapter = ?")
            .all(ctx.novelId, chapter);
    }
    else if (from !== null && to !== null) {
        rows = ctx.db
            .prepare("SELECT * FROM chapter_summaries WHERE novel_id = ? AND chapter BETWEEN ? AND ? ORDER BY chapter")
            .all(ctx.novelId, from, to);
    }
    else {
        rows = ctx.db
            .prepare("SELECT * FROM chapter_summaries WHERE novel_id = ? ORDER BY chapter")
            .all(ctx.novelId);
    }
...
// ============================================================
export async function novelCharacterState(args, ctx) {
    const characterUid = args["character_uid"];
    if (typeof characterUid !== "string" || !characterUid.trim()) {
        return singleError("character_uid", "non-empty string", describe(characterUid), "传入角色 character_uid（取自角色档案 character_identity）");
    }
    const at = asPositiveInt(args["at_chapter"]) ?? latestChapter(ctx) ?? 1;
    const uid = characterUid.trim();
    const rows = ctx.db
        .prepare(`SELECT predicate, object, from_chapter FROM facts
       ...
       ORDER BY predicate ASC, ${FACT_LATEST_ORDER_SQL}`)
        .all(ctx.novelId, uid, at, at);
    const card = {};
    for (const row of rows) {
        if (!(row.predicate in card))
            card[row.predicate] = row.object;
    }
    return {
        ok: true,
        character_uid: uid,
...
function readCharacterStatus(ctx, characterUid, atChapter) {
    const factRow = ctx.db
       ...
       LIMIT 1`)
        .get(ctx.novelId, characterUid, atChapter, atChapter);
    const factStatus = factRow?.object?.trim();
    if (factStatus)
        return factStatus;
    const cardRow = ctx.db
        .prepare(`SELECT card_json FROM character_cards
       WHERE novel_id = ? AND character_uid = ? AND as_of_chapter <= ?
       ORDER BY as_of_chapter DESC
       LIMIT 1`)
        .get(ctx.novelId, characterUid, atChapter);
    if (!cardRow?.card_json)
        return null;
    try {
        const card = JSON.parse(cardRow.card_json);
...
export async function novelCharacterStatuses(args, ctx) {
    const rawUids = args["character_uids"];
    if (!Array.isArray(rawUids)) {
        return singleError("character_uids", "string[]（角色 character_uid 列表）", describe(rawUids), "传入 character_uids 数组（取自角色档案 character_identity）");
    }
...
// ============================================================
export async function novelRelationship(args, ctx) {
    ...
    if (typeof uidA !== "string" || !uidA.trim() || typeof uidB !== "string" || !uidB.trim()) {
        return singleError("character_a_uid / character_b_uid", "两个非空 character_uid", `character_a_uid=${describe(uidA)}, character_b_uid=${describe(uidB)}`, "传入两个角色的 character_uid（取自角色档案 character_identity）");
    }
    const a = uidA.trim();
    const b = uidB.trim();
    const at = asPositiveInt(args["at_chapter"]);
    const rows = ctx.db
        .prepare(`SELECT object, event_chapter, from_chapter, invalidated_at_chapter FROM facts
...
const ACTION_RANK = { plant: 1, develop: 2, reveal: 3 };
/** 每条伏笔截至 upTo 章的最新已兑现动作（同章取生命周期更靠后的动作） */
function latestRealizedActions(ctx, upTo) {
    const rows = (upTo !== null
        ? ctx.db
            .prepare(`SELECT foreshadowing_id, chapter, action FROM foreshadowing_actions_log
...
export async function novelForeshadowingStatus(args, ctx) {
    const active = args["active"];
    const chapter = asPositiveInt(args["chapter"]);
    const registry = ctx.db
        .prepare(`SELECT * FROM foreshadowing_registry WHERE novel_id = ? ORDER BY planted_chapter, id`)
        .all(ctx.novelId);
...
// ============================================================
function buildDensityResult(scope, scopeId, expected, actual) {
    let warning = null;
    if (expected > 0) {
        if (actual < expected * 0.8)
            warning = "density_below_target";
        else if (actual > expected * 1.5)
            warning = "density_above_target";
    }
    return {
        ok: true,
        scope,
...
export async function novelForeshadowingDensity(args, ctx) {
    const chapter = asPositiveInt(args["chapter"]);
    const arcId = typeof args["arc_id"] === "string" ? args["arc_id"] : null;
    const volume = asPositiveInt(args["volume"]);
    const countByStatus = (start, end, status) => ctx.db
        .prepare(`SELECT COUNT(*) AS cnt FROM foreshadowing_actions_log
           WHERE novel_id = ? AND chapter BETWEEN ? AND ? AND status = ?`)
        .get(ctx.novelId, start, end, status).cnt;
    if (chapter !== null) {
        return buildDensityResult("chapter", chapter, countByStatus(chapter, chapter, "planned"), countByStatus(chapter, chapter, "realized"));
    }
    if (arcId !== null) {
        const arc = ctx.db
            .prepare(`SELECT chapter_start, chapter_end FROM arc_meta WHERE novel_id = ? AND arc_id = ?`)
            .get(ctx.novelId, arcId);
        if (!arc) {
            return singleError("arc_id", "已注册的 arc_id", arcId, "该 arc 不存在：先用 novel_get_arc 确认 arc_id，或先提交书级大纲");
        }
        return buildDensityResult("arc", arcId, countByStatus(arc.chapter_start, arc.chapter_end, "planned"), countByStatus(arc.chapter_start, arc.chapter_end, "realized"));
    }
    if (volume !== null) {
        const range = getVolumeRange(ctx, volume);
        if (!range)
            return buildDensityResult("volume", volume, 0, 0);
        return buildDensityResult("volume", volume, countByStatus(range.chapter_start, range.chapter_end, "planned"), countByStatus(range.chapter_start, range.chapter_end, "realized"));
    }
    return singleError("scope", "chapter / arc_id / volume 三选一", "全部缺失", "传入 chapter（章号）、arc_id 或 volume（卷号）之一");
}
// ============================================================
// novel_get_arc — 按 arc_id 或章号查 arc 元信息与卷边界
// ============================================================
function arcResponse(ctx, row, queriedChapter) {
...
export async function novelGetArc(args, ctx) {
    const arcId = typeof args["arc_id"] === "string" ? args["arc_id"] : null;
    const chapter = asPositiveInt(args["chapter"]);
    if (arcId !== null) {
        const row = ctx.db
            ...
            .get(ctx.novelId, arcId);
        if (!row)
            return { ok: true, found: false, arc_id: arcId, message: "arc 不存在" };
    ...
    return singleError("arc_id / chapter", "二选一", "全部缺失", "传入 arc_id（如 \"V01-A02\"）或 chapter（章号）之一");
}
// ============================================================
 ...
 */
export async function novelCheckProseHygiene(args, ctx) {
    const chapter = asPositiveInt(args["chapter"]);
    if (chapter === null) {
        return singleError("chapter", "正整数（被扫描的章节号）", describe(args["chapter"]), "传本章章号，例如 chapter: 12");
    }
    if (!ctx.projectRoot) {
        return singleError("projectRoot", "可用的项目根目录", "缺失", "NOVEL_CONFIG_PATH 未指向有效项目");
    }
    const manuscript = await resolveWorkingManuscript(ctx.projectRoot, chapter);
    if (manuscript === null) {
        return singleError("manuscript", "本章正文存在（草稿区或正式路径）", "未找到本章正文文件", "先完成正文生成（/write 步骤 3）再扫描散文洁净度");
    }
...
export async function novelGetReview(args, ctx) {
    const chapter = asPositiveInt(args["chapter"]);
    if (chapter === null) {
        return singleError("chapter", "integer ≥ 1", describe(args["chapter"]), "传入被审章节号（正整数）");
    }
    const row = ctx.db
...
// ============================================================
export async function novelFailedReviews(_args, ctx) {
    const rows = ctx.db
        .prepare(`SELECT chapter FROM chapter_reviews
...
// ============================================================
export async function novelGetStructureBudget(_args, ctx) {
    if (!ctx.estimatedTotalChapters || !ctx.wordsPerChapter) {
        return singleError("config.yaml", "estimated_total_chapters 与 words_per_chapter 均已设置", `estimated_total_chapters=${ctx.estimatedTotalChapters ?? "缺失"}, words_per_chapter=${ctx.wordsPerChapter ?? "缺失"}`, "config.yaml 缺篇幅字段：先执行 /narracat:setup 收集预计总章数与每章字数");
    }
...
// ============================================================
export async function novelListStructureCards(args, ctx) {
    const stage = args["stage"];
    if (typeof stage !== "string" || !STRUCTURE_STAGES.includes(stage)) {
        return singleError("stage", `"${STRUCTURE_STAGES.join("\" | \"")}" 三选一`, describe(stage), "传入规划所处阶段：stage-1（书级 storylines/伏笔注册表/arc）/ stage-2（章级细纲）/ stage-opening（全书开局）");
    }
    ...
    try {
        writePlanningCapabilityReceipt(ctx.projectRoot, stage, stageCards.map(({ id, source_pack_id, source_pack_version, origin, dimension, one_line }) => ({
            card_id: id,
...
function getPreviousBriefs(ctx, chapter) {
    const from = Math.max(1, chapter - 3);
    const to = chapter - 1;
    if (to < from)
        return [];
    const rows = ctx.db
        .prepare(`SELECT * FROM chapter_summaries
 ...
 */
async function widenLatestEndingSnippet(ctx, briefs) {
 ...
 */
export function isChapterCommitted(ctx, chapter) {
    return (ctx.db
 ...
 */
async function pickAutoVoiceSample(ctx, chapter) {
    for (let prev = chapter - 1; prev >= 1 && prev >= chapter - 3; prev -= 1) {
        if (!isChapterCommitted(ctx, prev))
            continue;
        if ((await checkReviewFreshness(ctx, prev, true)) !== null)
            continue;
        ...
        for (const para of paragraphs) {
            ...
            if (Array.from(buffer).length >= AUTO_SAMPLE_MIN_CHARS)
                break;
        }
        const chars = Array.from(buffer);
        if (chars.length < AUTO_SAMPLE_MIN_CHARS)
            continue;
 ...
 */
async function buildBookVoiceExamples(ctx, chapter, buildNotes) {
    let anchors;
    try {
        anchors = listStyleAnchorRows(ctx).slice(0, 2);
    }
    catch (err) {
        buildNotes.push(`样章锚表读取失败，本次退回纯跨书范例：${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
    if (anchors.length > 0) {
    ...
    if (ctx.styleAnchorAutoFallback !== true)
        return [];
...
function getArcSummariesForPack(ctx, chapter, currentVolume) {
    const rows = ctx.db
        .prepare(`SELECT * FROM arc_summaries
       WHERE novel_id = ? AND chapter_end < ? ORDER BY chapter_start`)
        .all(ctx.novelId, chapter);
    const arcVolume = new Map();
    if (currentVolume !== null) {
        const arcs = ctx.db
            .prepare(`SELECT arc_id, volume_no FROM arc_meta WHERE novel_id = ?`)
            .all(ctx.novelId);
        for (const a of arcs)
            arcVolume.set(a.arc_id, a.volume_no);
    }
    return rows
        .filter((row) => {
        if (row.scope === "volume")
...
function getCharacterCardsForPack(ctx, characters, asOfChapter) {
    const cards = [];
    for (const { uid, name } of characters) {
        ...
        // 且存量卡陈旧/超前的场景。
        const stored = ctx.db
            .prepare(`SELECT * FROM character_cards WHERE novel_id = ? AND character_uid = ? AND as_of_chapter <= ?`)
            .get(ctx.novelId, uid, asOfChapter);
        if (!stored)
            continue;
        try {
            const card = JSON.parse(stored.card_json);
 ...
 */
function getCharacterRelationshipsForPack(ctx, characters, asOfChapter) {
    ...
    const rows = ctx.db
        .prepare(`SELECT subject_character_uid AS a_uid, subject_character_b_uid AS b_uid, object, from_chapter
       FROM facts
       WHERE novel_id = ? AND predicate = 'relationship'
         ...
         AND ${FACT_VALID_AT_SQL}
       ORDER BY ${FACT_LATEST_ORDER_SQL}`)
        .all(ctx.novelId, ...uids, ...uids, asOfChapter, asOfChapter);
    const seen = new Set();
    const relationships = [];
    for (const row of rows) {
    ...
    relationships.sort((l, r) => l.character_a.localeCompare(r.character_a, "zh") ||
 ...
 */
export function extractReferencedForeshadowingIds(chapterOutlineText) {
 ...
 */
export function getForeshadowingDue(ctx, chapter, arc, volumeRange, currentVolume, referencedIds = []) {
    const registry = ctx.db
        .prepare(`SELECT * FROM foreshadowing_registry WHERE novel_id = ? ORDER BY planted_chapter, id`)
        .all(ctx.novelId);
    const latest = latestRealizedActions(ctx, null);
    const within = (value, range) => value !== null &&
        range !== null &&
    ...
    const arcRange = arc
        ...
        : null;
    const active = registry.filter((row) => latest.get(row.id)?.action !== "reveal");
    // spec §4.1 P1：未种下（planted_chapter 晚于目标章）的伏笔是未来计划，
    // 不进 due 也不进计数——写手不该看到还没发生的秘密（剧透 + 催兑现双重污染）。
    // planted_chapter 可空时保守放行（视为已存在）。
    const planted = active.filter((row) => row.planted_chapter === null ||
    ...
    const due = planted.filter((row) => {
        ...
        if (target !== null && target <= chapter + 10)
            return true;
        if (row.type === "medium") {
            return (within(row.planted_chapter, volumeRange) ||
...
function getThroughLineAnchor(ctx, chapter) {
    ...
    for (const predicate of ENGINE_FACT_PREDICATES) {
        ...
        if (row && row.object.trim())
            engine_fields.push({ key: predicate, value: row.object });
    }
    // 贯穿故事线：作者标注、未收线
    const through_storylines = ctx.db
        .prepare(`SELECT id, name, type FROM storylines
         WHERE novel_id = ? AND is_through_line = 1 AND status != 'resolved'
         ORDER BY priority, id`)
        .all(ctx.novelId).map((r) => ({ id: r.id, name: r.name, type: r.type }));
    ...
    const latest = latestRealizedActions(ctx, null);
    const core_foreshadowing = ctx.db
        .prepare(`SELECT id, description, planted_chapter, target_reveal FROM foreshadowing_registry
...
function getStorylineFocus(ctx, chapter) {
    return ctx.db
        .prepare(`SELECT s.id, s.name, s.type
...
// ============================================================
export async function novelWritingContext(args, ctx) {
    const chapter = asPositiveInt(args["chapter"]);
    if (chapter === null) {
        return singleError("chapter", "integer ≥ 1", describe(args["chapter"]), "传入即将撰写的章节号（正整数）");
    }
    ...
    for (const brief of briefs) {
        const row = ctx.db
            .prepare(`SELECT characters FROM chapter_summaries WHERE novel_id = ? AND chapter = ?`)
            .get(ctx.novelId, brief.chapter);
        for (const ref of parseCharacterRefs(row?.characters))
            characters.set(ref.uid, ref);
    }
    // 伏笔白名单：与 WCP builder 同构——本章细纲结构化 foreshadowing_touch 优先，
    // .json 缺失/解析失败（旧书）或本章尚未细化时回退全文正则兜底
    let referencedForeshadowingIds = [];
    if (ctx.projectRoot) {
    ...
    return {
        ...
        character_cards: getCharacterCardsForPack(ctx, [...characters.values()], Math.max(1, chapter - 1)),
        foreshadowing_due: getForeshadowingDue(ctx, chapter, arc, volumeRange, arc?.volume_no ?? null, referencedForeshadowingIds),
...
async function findChapterOutlineFile(projectRoot, chapter, extension = "md") {
...
export async function findChapterManuscriptFile(projectRoot, chapter) {
    const padded = chapterFileSegment(chapter);
 ...
 */
async function readChapterOutlineForPack(ctx, mdPath) {
    const jsonPath = mdPath.replace(/\.md$/, ".json");
    try {
        const ch = JSON.parse(await readFile(jsonPath, "utf-8"));
        const storylineNames = new Map(ctx.db
            .prepare(`SELECT id, name FROM storylines WHERE novel_id = ?`)
            .all(ctx.novelId).map((r) => [r.id, r.name]));
        const foreshadowingDescriptions = new Map(ctx.db
            .prepare(`SELECT id, description FROM foreshadowing_registry WHERE novel_id = ?`)
            .all(ctx.novelId).map((r) => [r.id, r.description]));
        const touchIds = [
            ...new Set((ch.foreshadowing_touch ?? [])
                .map((t) => t.id)
...
export function parseRenderedOutline(text) {
...
async function readNarratorVoice(projectRoot) {
    let content;
    try {
 ...
 */
async function readWorldRules(projectRoot) {
    let content;
    try {
        content = await readFile(join(projectRoot, "bible", "premise.md"), "utf-8");
    }
    ...
    const lines = content.replace(/\r\n?/g, "\n").split("\n");
    const start = lines.findIndex((line) => /^#{2,4}\s*.*世界规则/.test(line.trim()));
    if (start === -1)
        return null;
    const headingLevel = (lines[start].match(/^#+/) ?? ["##"])[0].length;
    let end = lines.length;
 ...
 * 导出供单测。
 */
export function positivizeNarratorFreeText(text) {
    if (!RESTRAINT_RE.test(text))
        return [text, false];
    let dropped = false;
    const parts = splitClauses(text);
    for (let i = 0; i < parts.length; i += 2) {
        const clause = parts[i];
        if (!RESTRAINT_RE.test(clause))
            continue;
...
export function positivizeProsody(text) {
    if (!PROSODY_TEST_RE.test(text))
        return [text, false];
    let dropped = false;
    const parts = splitClauses(text);
    for (let i = 0; i < parts.length; i += 2) {
        const clause = parts[i];
        if (!clause || !PROSODY_TEST_RE.test(clause))
            continue;
        dropped = true;
...
export function renderStyleDirective(voice, styleProfile, warnings) {
    ...
    if (voice) {
        ...
        if (address)
            dims.push(`以${narratorAddressPhrase(address)}叙述`);
        if (dims.length > 0)
 ...
 */
async function getSemanticContext(ctx, chapter, parsed, storylineNames, dueDescriptions, excludeTexts = []) {
    const queries = [];
    const push = (q) => {
    ...
    for (const p of parsed.pressure_points.slice(0, 2))
        push(p);
    for (const name of parsed.characters.slice(0, 2))
        push(name);
    ...
    try {
        ...
        return hits
            ...
            const item = {
                content: detail.content,
                score: Number(decayScore(hit.rrfScore, detail.chapter, chapter).toFixed(6)),
            };
            if (detail.chapter !== null)
                item.source_chapter = detail.chapter;
            return [item];
        })
            .filter((item) => {
...
const FUNCTION_CHARS = new Set("的了和在是有对与着过地得之其也而就都把被让向从这那个一不你我他她它们之乎者也".split(""));
/** 抽象词占比：cross-chapter-warnings §2 算法（导出供单测） */
export function abstractRatio(text) {
    const meaningful = [...text].filter((ch) => /[一-鿿]/.test(ch) && !FUNCTION_CHARS.has(ch));
...
function getCrossChapterWarnings(ctx, chapter, briefs, foreshadowingDue) {
    const out = [];
    // 4.3 锚点泛泛
    const withAnchor = briefs.filter((b) => b.core_experience && b.core_experience.trim());
    if (withAnchor.length >= 2) {
        const scored = withAnchor
            .map((b) => ({ text: b.core_experience, ratio: abstractRatio(b.core_experience) }))
    ...
    if (dueSoon.length > 0 && chapter > FORESHADOW_SETUP_WINDOW) {
        ...
        const to = chapter - 1;
        const row = ctx.db
            .prepare(`SELECT COUNT(*) AS c FROM foreshadowing_actions_log
         WHERE novel_id = ? AND chapter BETWEEN ? AND ? AND action IN ('develop', 'reveal')`)
            .get(ctx.novelId, from, to);
        if (row.c === 0) {
...
export async function novelBuildWritingContextPack(args, ctx) {
    const chapter = asPositiveInt(args["chapter"]);
    if (chapter === null) {
        return singleError("chapter", "integer ≥ 1", describe(args["chapter"]), "传入即将撰写的章节号（正整数）");
    }
    if (!ctx.projectRoot) {
        return singleError("projectRoot", "可用的项目根目录", "缺失", "NOVEL_CONFIG_PATH 未指向有效项目：检查 MCP 配置后重试");
    }
    const outlinePath = await findChapterOutlineFile(ctx.projectRoot, chapter);
    if (!outlinePath) {
        return singleError("chapter", `outline/vol-VV/ch-${chapterFileSegment(chapter)}.md 存在`, "细纲文件未找到", `第${chapter}章细纲不存在：先执行 /narracat:plan 细化本章`);
    }
    ...
    for (const note of packPools.notes)
        buildNotes.push(note);
    ...
    while (styleExamples.length > minKeep &&
    ...
    while (estimateBlockTokens(throughLineAnchor) > BLOCK_BUDGETS.through_line_anchor &&
    ...
    const cardCharacterMap = new Map();
    ...
    for (const name of parsed.characters) {
        ...
        if (uid)
            cardCharacterMap.set(uid, { uid, name: normalizeName(name, cardAliasMap) });
    }
    ...
    const cardRows = ctx.db
       ...
       WHERE novel_id = ? AND chapter BETWEEN ? AND ?`)
        .all(ctx.novelId, Math.max(1, chapter - 3), chapter - 1);
    for (const row of cardRows) {
        for (const ref of parseCharacterRefs(row.characters)) {
            if (!cardCharacterMap.has(ref.uid))
                cardCharacterMap.set(ref.uid, ref);
        }
    }
    ...
    const plannedStateRows = ctx.db
        .prepare(`SELECT character_name, dimension, operation, value, reason FROM planned_state_changes
       WHERE novel_id = ? AND chapter = ? AND status = 'planned'
       ORDER BY rowid ASC`)
        .all(ctx.novelId, chapter);
    const plannedVocab = loadStateVocabulary(ctx.projectRoot);
    const plannedDimNames = new Map((plannedVocab?.dimensions ?? []).map((d) => [d.key, d.display_name]));
    ...
    while (plannedStateChanges.length > 1 &&
    ...
    const characterRelationships = getCharacterRelationshipsForPack(ctx, [...cardCharacterMap.values()], Math.max(1, chapter - 1));
    let droppedRelationships = 0;
    while (characterRelationships.length > 1 &&
    ...
    const canonicalNameByUid = new Map();
    ...
    const { edges: relationshipEdges, directPairKeys } = loadValidRelationshipEdges(ctx, Math.max(1, chapter - 1));
    ...
    while (derivedRelationships.length > 0 &&
    ...
    const foreshadowingDue = getForeshadowingDue(ctx, chapter, arc, volumeRange, currentVolume, touchIds ?? extractReferencedForeshadowingIds(chapterOutline));
    // major 优先保留：major 排前、non-major 排后；超预算时只 pop 末尾的 non-major，
    // 直到 non-major 耗尽才允许 pop major（核心伏笔豁免距离衰减，不因预算溢出被丢）
    const TYPE_RANK = { major: 0, medium: 1, small: 2 };
    foreshadowingDue.due.sort((a, b) => (TYPE_RANK[a.type] ?? 9) - (TYPE_RANK[b.type] ?? 9));
    let foreshadowingToCount = 0;
    while (foreshadowingDue.due.length > 1 &&
    ...
    const semanticExcludeTexts = [];
    for (const brief of briefs) {
        semanticExcludeTexts.push(brief.summary);
        if (brief.opening_snippet)
            semanticExcludeTexts.push(brief.opening_snippet);
        if (brief.ending_snippet)
            semanticExcludeTexts.push(brief.ending_snippet);
    }
    for (const card of characterCards) {
        semanticExcludeTexts.push(...Object.values(card.card));
    }
    let semanticContext = await getSemanticContext(ctx, chapter, parsed, storylineFocus.map((s) => s.name), foreshadowingDue.due.map((f) => f.description), semanticExcludeTexts);
    while (semanticContext.length > 0 &&
    ...
    try {
        ...
        buildNotes.push(`章级能力回执写入失败：${err instanceof Error ? err.message : String(err)}`);
    }
    const pack = {
        target_chapter: chapter,
    ...
    const packPath = join(ctx.projectRoot, ".narracat", "context-packs", `ch-${chapterFileSegment(chapter)}.json`);
 ...
 */
export async function novelExtractionScaffold(args, ctx) {
    const chapter = asPositiveInt(args["chapter"]);
    if (chapter === null) {
        return singleError("chapter", "正整数（当前抽取的章节号）", describe(args["chapter"]), "传当前抽取章号，例如 chapter: 5");
    }
    if (!ctx.projectRoot) {
        return singleError("projectRoot", "可用的项目根目录", "缺失", "NOVEL_CONFIG_PATH 未指向有效项目");
    }
    ...
    const known_facts_summary = ctx.db
        .prepare(`SELECT subject, predicate, object, from_chapter FROM facts
       ...
       LIMIT 30`)
        .all(ctx.novelId, chapter, chapter);
...
export async function novelGetCharacterDialogueSamples(args, ctx) {
    const characterUid = typeof args["character_uid"] === "string" ? args["character_uid"].trim() : "";
    if (!characterUid) {
        return singleError("character_uid", "非空 character_uid", describe(args["character_uid"]), "必须提供要查询台词的角色 character_uid（机器主键，见 bible/characters/ 档案的 character_identity）");
    }
    const chapterStart = asPositiveInt(args["chapter_start"]);
    const chapterEnd = asPositiveInt(args["chapter_end"]);
    const dialogueType = typeof args["dialogue_type"] === "string" &&
        DIALOGUE_TYPE_VALUES.has(args["dialogue_type"])
        ? args["dialogue_type"]
        : null;
    const limit = asPositiveInt(args["limit"]) ?? 20;
    const where = ["novel_id = ?", "character_uid = ?"];
    const params = [ctx.novelId, characterUid];
    ...
    const total = ctx.db
        ...
        .get(...params).c;
    const rows = ctx.db
        .prepare(`SELECT chapter, dialogue_text, dialogue_type, context, emotion, position_in_chapter
...
// ============================================================
export async function novelPackAuthoringVocab() {
...
// ============================================================
export async function novelPackAuthoringPreview(args) {
    ...
    if (type !== "persona" && type !== "craft") {
        return singleError("card.type", "persona | craft", String(type), "structure 卡装载预览由 App 本地映射；本工具只接受 card.type=persona 或 craft");
    }
    ...
    if (type === "craft") {
        ...
        if (!Array.isArray(triggers) || !triggers.every((t) => typeof t === "string")) {
            return singleError("card.triggers", "字符串数组", JSON.stringify(triggers), "craft 卡需提供 triggers: string[]（选卡命中的触发词）");
        }
        const emotionTags = c["emotion_tags"];
        if (!Array.isArray(emotionTags) || !emotionTags.every((t) => typeof t === "string")) {
            return singleError("card.emotion_tags", "字符串数组", JSON.stringify(emotionTags), "craft 卡需提供 emotion_tags: string[]（无情绪标签传空数组 []）");
        }
        const exclusions = c["exclusions"];
        if (!Array.isArray(exclusions) || !exclusions.every((t) => typeof t === "string")) {
            return singleError("card.exclusions", "字符串数组", JSON.stringify(exclusions), "craft 卡需提供 exclusions: string[]（无排除词传空数组 []）");
        }
    ...
    if (!Array.isArray(keywords) || !keywords.every((k) => typeof k === "string")) {
        return singleError("card.keywords", "字符串数组", JSON.stringify(keywords), "persona 卡需提供 keywords: string[]（叙述声音关键词）");
    }
