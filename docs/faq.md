...
| `error_occurred` | 出错 | 错误码（我自己定义的 8 个枚举之一）；如果是 Agent 运行失败，再加一个失败原因分类（13 个枚举之一，例如 `network-interrupted` 网络中途断开、`provider-auth` 鉴权失败、`context-overflow` 上下文超长）。**两者都是我预先定义好的枚举码，不含原始报错文本与堆栈** |
