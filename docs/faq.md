...
| `error_occurred` | Agent 任务失败时（写章节、立项卡、大纲等都算） | 错误码（我自己定义的 8 个枚举之一）+ 出错的功能模块（上面那 18 个之一；世界观与自由对话目前记作 `unknown`）+ 失败原因分类（13 个枚举之一，例如 `network-interrupted` 网络中途断开、`provider-auth` 鉴权失败、`context-overflow` 上下文超长）。**三者都是我预先定义好的枚举码，不含原始报错文本与堆栈** |
